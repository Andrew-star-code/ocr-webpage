...
class LlamaCppVisionBackend:
    ...
    def __init__(self, base_url, model, timeout=600, concurrency=1, max_retries=2, client=None):
        ...
        self._structured_verified: set[str] = set()
        self._vision_verified: set[str] = set()
            ...
            try:
                ...
                if response.status_code in {429, 500, 502, 503, 504}:
                    ...
                    if not text.strip():
                        ...
                        requested_model = options.model or self.default_model
                        self._vision_verified.add(requested_model)
                        if options.supports_json_schema:
                            try:
                                ...
                                structured = False
                            if structured:
                                self._structured_verified.add(requested_model)
    ...
    async def get_model_info(self, model=None):
        ...
        return ModelInfo(
            name=name,
            present=present,
            vision_capable=present and name in self._vision_verified,
            structured_output=present and name in self._structured_verified,
        )

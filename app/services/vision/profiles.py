...
class ModelProfile(BaseModel):
    ...
    def valid_backend(cls, value):
        if value not in {"ollama", "llama_cpp"}:
    ...
    def valid_tiling(cls, value):
        if value not in {"full_page", "tiles", "full_page_plus_tiles"}:
            raise ValueError("unsupported tiling strategy")
        return value
    ...
    def normalized_coordinates(cls, value):
        if value != "normalized":
            raise ValueError("only normalized coordinates are supported")
        return value
...
class ProfileRegistry:
    ...
    def get(self, name: str) -> ModelProfile:
        try:
            ...
            raise ServiceError(
                "model_profile_not_found", "Model profile not found", 404, {"profile": name}
            ) from exc

    def public(self):
        return [
            p.model_dump(exclude={"system_prompt", "user_prompt"}) for p in self._profiles.values()
        ]

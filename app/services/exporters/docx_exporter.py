    ...
    def export(self, result, options):
        ...
        for pi, page in enumerate(result.document.pages):
            ...
            for b in sorted(page.blocks, key=lambda x: x.reading_order):
                ...
                if isinstance(b, HeadingBlock) and value:
                    p = word.add_heading(value, level=b.heading_level)
                elif isinstance(b, ListBlock):
                    for item in b.items:
                        level = min(max(item.level, 0), 8)
                    ...
                    for row in b.rows:
                        for c in row.cells:
                            if (c.row_index, c.column_index) in occupied:
                                continue
                            cell = table.cell(c.row_index, c.column_index)
                            cell.text = c.text
                            end = (c.row_index + c.row_span - 1, c.column_index + c.column_span - 1)
                            if end != (c.row_index, c.column_index):
                                cell.merge(table.cell(*end))
                            occupied.update(
                                (r, col)
                                for r in range(c.row_index, c.row_index + c.row_span)
                                for col in range(c.column_index, c.column_index + c.column_span)
                            )
                            if c.is_header:
                    ...
                    label = {"checkbox": "[Флажок]", "signature": "[Подпись]", "stamp": "[Печать]"}[
                        b.type
                    ]
                    p = word.add_paragraph(label + (f" {value}" if value else ""))
                elif value:
                    p = word.add_paragraph(value)
                else:
                    continue
                if options.preserve_layout:
                    p.alignment = _ALIGN[b.style.alignment]
                    for run in p.runs:
                        run.bold = b.style.bold
                        run.italic = b.style.italic
                        run.underline = b.style.underline
        word.save(output)

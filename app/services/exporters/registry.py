...
class ExporterRegistry:
 def __init__(self):self._items={e.format:e for e in (JsonExporter(),DocxExporter(),TxtExporter(),MarkdownExporter(),HtmlExporter(),SearchablePdfExporter())}
 def get(self,name):return self._items.get(name)
 def formats(self):return [{"format":e.format,"mime_type":e.mime_type,"extension":e.extension} for e in self._items.values()]

...
def _duplicate(a, b):
    ...
    return SequenceMatcher(None, ta, tb).ratio() > 0.92 and _iou(a.bbox, b.bbox) > 0.35
...
def merge_pages(base, partials):
    ...
    for page in partials:
        for candidate in page.blocks:
            ...
            elif len(candidate.original_text) > len(matches[0].original_text):
                details[details.index(matches[0])] = candidate

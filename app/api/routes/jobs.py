...
async def create_job(
    ...
    language: str = Form("rus+eng", max_length=32),
    model_profile: str = Form("default", max_length=64),
    ...
    if not exporter:
        raise ServiceError("invalid_request", "Unknown output format", 400)
    ...
    store = JobStore(settings.redis_url, settings.job_metadata_ttl_seconds, redis)
    capacity = QueueCapacity(redis, settings.max_queue_size, settings.document_processing_timeout)
    job_id = str(uuid.uuid4())
    stored = None
    try:
        if not await capacity.reserve(job_id):
            raise ServiceError("queue_is_full", "Processing queue is full", 429)
        data = await read_upload(file, settings.max_upload_size)
        mime = validate_document(data, settings)
        stored = await request.app.state.storage.save_input(job_id, data, mime)
        await request.app.state.storage.tag_job_file(job_id, stored)
        options = {
        ...
        try:
            ...
            await request.app.state.storage.delete_job_files(job_id)
            await capacity.release(job_id)
            raise ServiceError("queue_dispatch_failed", "Could not dispatch job", 503) from exc
        return {key: value[key] for key in ("job_id", "status", "created_at", "version")}
    except Exception:
        job_exists = await store.get(job_id)
        if stored and not job_exists:
            await request.app.state.storage.delete_job_files(job_id)
        if not job_exists:
            await capacity.release(job_id)
        raise
...
async def status(job_id: str):
    settings = get_settings()
    store = JobStore(settings.redis_url, settings.job_metadata_ttl_seconds)
    try:
        ...
        return {
            ...
            if key not in {"options", "input_file", "result_file"}
        }
    finally:
        await store.close()
...
async def result(request: Request, job_id: str):
    ...
    try:
        ...
        if not value["result_file"]:
            raise ServiceError("result_expired", "Result expired", 410)
        body = await request.app.state.storage.read_result(value["result_file"])
        exporter = request.app.state.exporters.get(value["options"]["output_format"])
        return Response(
            body,
...
async def cancel(job_id: str):
    ...
    store = JobStore(settings.redis_url, settings.job_metadata_ttl_seconds)
    try:
        value = await store.cancel(job_id)
        if not value:
            raise ServiceError("job_not_found", "Job not found", 404)
        return {"job_id": job_id, "status": value["status"]}
    finally:
        await store.close()
...
async def delete(request: Request, job_id: str):
    ...
    try:
        ...
        capacity = QueueCapacity(
            store.redis, settings.max_queue_size, settings.document_processing_timeout
        )
        await capacity.release(job_id)
        await request.app.state.storage.delete_job_files(job_id)

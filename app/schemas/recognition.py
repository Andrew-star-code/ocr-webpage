...
class TableBlock(BlockBase):
    ...
    def dimensions(self):
        ...
        for row in self.rows:
            for c in row.cells:
                if (
                    c.row_index + c.row_span > self.row_count
                    or c.column_index + c.column_span > self.column_count
                ):
                    raise ValueError("table cell outside dimensions")
                for position in (
                    (r, col)
...
DocumentBlock = Annotated[
    ParagraphBlock | HeadingBlock | ListBlock | TableBlock | GenericBlock,
...
class PageRecognition(BaseModel):
    ...
    def unique_order(self):
        ...
        if len(orders) != len(set(orders)):
            raise ValueError("duplicate reading_order")
        return self

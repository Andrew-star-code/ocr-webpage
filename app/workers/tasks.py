...
async def _publish_result(
    ...
    if current["status"] in {"completed", "completed_with_warnings"}:
        if current.get("result_file") == stored.identifier:
            return current
        await _delete_result_artifacts(storage, job_id, stored.identifier)
        raise JobStateConflict("A different result was already published")

...
def test_magic_validation_and_unknown():
    ...
    assert validate_document(png(), Settings()) == "image/png"
...
def test_bbox_and_unique_ids():
    with pytest.raises(ValidationError):
        BoundingBox(x1=0.5, y1=0, x2=0.2, y2=1)
    base = {
        "page_number": 1,
        "width": 1,
        "height": 1,
        "blocks": [
            {"id": "x", "type": "paragraph", "reading_order": 1},
            {"id": "x", "type": "paragraph", "reading_order": 2},
        ],
    }
    with pytest.raises(ValidationError):
        PageRecognition.model_validate(base)
...
def test_tile_coordinates_and_overlap_aware_merge():
    tiles = make_tiles(png((2000, 1000)), 1000, 100)
    assert len(tiles) == 3 and tiles[1].x == 900
    local = PageRecognition(
        page_number=1,
        width=1000,
        height=1000,
        blocks=[
            ParagraphBlock(
                id="a",
                reading_order=1,
                original_text="Длинный повторяющийся текст",
                bbox=BoundingBox(x1=0.1, y1=0.1, x2=0.5, y2=0.2),
            )
        ],
    )
    transformed = transform_page(local, tiles[1])
    assert transformed.blocks[0].bbox.x1 == pytest.approx(0.5)
    far = local.model_copy(
        update={
            "blocks": [
                local.blocks[0].model_copy(
                    update={"id": "b", "bbox": BoundingBox(x1=0.6, y1=0.6, x2=0.9, y2=0.7)}
                )
            ]
        }
    )

...
def fixture_result():
    ...
    page = PageRecognition(
        ...
        blocks=[
            ParagraphBlock(
                ...
                reading_order=1,
                original_text="Тест",
                bbox=BoundingBox(x1=0.1, y1=0.1, x2=0.9, y2=0.3),
            )
        ],
    )
    ...
    return RecognitionResult(doc, [image], [RenderedPage(1, image, 200, 100, 300, 0, 200, 100)])
...
def upload():
    ...
    return {"file": ("unsafe/../../x.png", stream.getvalue(), "image/png")}
...
def test_json_flags_and_api_key():
    with TestClient(app) as client:
        ...
        denied = client.post("/v1/ocr", files=upload())
        ...
        body = response.json()
        assert "metadata" not in body and "bbox" not in body["pages"][0]["blocks"][0]
...
def test_docx_searchable_pdf_and_validation():
    with TestClient(app) as client:
        ...
        pdf = client.post(
            "/v1/ocr", headers=headers, files=upload(), data={"output_format": "searchable_pdf"}
        )
        assert pdf.status_code == 200 and pdf.content.startswith(b"%PDF")
        assert (
            client.post(
                "/v1/ocr", headers=headers, files=upload(), data={"output_format": "bad"}
            ).status_code
            == 400
        )
        unknown = client.post(
            "/v1/ocr", headers=headers, files=upload(), data={"model_profile": "missing"}
        )
        assert (
            unknown.status_code == 404
            and unknown.json()["error"]["code"] == "model_profile_not_found"
        )

...
def test_json_flags_and_api_key():
 with TestClient(app) as client:
  app.state.pipeline=Pipeline();denied=client.post("/v1/ocr",files=upload());assert denied.status_code==401
...
def test_docx_searchable_pdf_and_validation():
 with TestClient(app) as client:
  app.state.pipeline=Pipeline();headers={"X-API-Key":"change-me"}
...
def test_profiles_endpoint_is_loaded_registry():
 with TestClient(app) as client:
  response=client.get("/v1/model/profiles",headers={"X-API-Key":"change-me"});assert response.status_code==200;names={p["name"] for p in response.json()["profiles"]};assert "default" in names and "system_prompt" not in response.text

...
def upload():
    ...
    return {"file": ("scan.png", stream.getvalue(), "image/png")}
...
def test_create_status_cancel_delete(monkeypatch, redis):
    ...
    with TestClient(app) as client:
        ...
        cancelled = client.post(f"/v1/jobs/{job}/cancel", headers=headers)
        assert cancelled.status_code == 200 and cancelled.json()["status"] == "cancelled"
        assert client.delete(f"/v1/jobs/{job}", headers=headers).status_code == 204
        assert redis.zcard("ocr:queue:jobs") == 0
...
def test_atomic_queue_full_and_dispatch_cleanup(monkeypatch, redis):
    ...
    redis.flushdb()
    monkeypatch.setattr(
        tasks.recognize_job, "send", lambda job_id: (_ for _ in ()).throw(RuntimeError("broker"))
    )
    with TestClient(app) as client:
        app.state.rate_limiter = Allow()
